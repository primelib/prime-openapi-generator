    ...
    companion object {
        fun of(openAPI: OpenAPI): OpenAPIDetails {
            ...
            openAPI.info?.let { info ->
                ...
                details.appDescription = StringUtils.defaultIfEmpty(
                    info.description,
            ...
            openAPI.paths?.let { paths ->
                paths.forEach { (path, pathItem) ->
                    pathItem.readOperations().forEach { operation ->
                        val hasFormParam =
                            if (operation.parameters?.any { it.`in` == "formData" } == true) {
                                true
                            } else if (operation.requestBody?.content?.containsKey("multipart/form-data") == true) {
                                true
                            } else if (operation.requestBody?.content?.containsKey("application/x-www-form-urlencoded") == true) {
                                true
                            } else {
                                false
                            }
